...
import { useState as B, useMemo as b, useRef as Fe, useEffect as Le, useLayoutEffect as at } from "react";
import f from "moment-timezone";
const Z = 64, $e = (t, s) => f.utc(t).tz(s), ut = ["HH:mm", "hh:mm A", "h:mm a", "H:mm", "h:mmA", "h:mma", "hh:mm a", "HH:mm:ss"], lt = (t) => f(t, ut, !0), ze = (t) => {
  const s = lt(t);
  return s.isValid() ? s.hours() * 60 + s.minutes() : 0;
}, st = (t) => {
  if (t?.length) {
    const s = t[0].start, a = t[t.length - 1].end;
    return {
      startMinutes: ze(s),
      endMinutes: ze(a)
    };
  ...
  const a = [], o = t.clone().startOf("day");
  for (let n = 0; n < 1440; n += s)
    a.push(o.clone().add(n, "minutes"));
  return a;
}, et = (t, s, a, o, n) => {
  ...
  const i = (m, h = !1) => {
    ...
    const g = t.hours() * 60 + t.minutes(), A = m.some((D) => {
      const N = ze(D.start), ee = ze(D.end);
      return g >= N && g < ee;
    });
    return h ? !A : A;
  };
  ...
  const o = [];
  return t.forEach((n) => {
    const k = $e(n.start, a), u = $e(n.end, a);
    if (n.allDay) {
      k.isSame(s, "day") && o.push({ ...n, start: k, end: u, allDay: !0 });
      return;
    }
    let i = k.clone().startOf("day");
    for (; i.isBefore(u); ) {
      const m = i.clone(), h = i.clone().endOf("day"), g = f.max(k, m), A = f.min(u, h);
      g.isSame(s, "day") && o.push({
        ...n,
        start: g,
        end: A
      }), i.add(1, "day");
  ...
  return o.map((u) => {
    ...
    const m = u.start, h = u.end, g = m.diff(s.clone().startOf("day"), "minutes") / a * Z, A = h.diff(m, "minutes") / a * Z;
    return {
      ...u,
      columnIndex: i,
      columnCount: k,
  ...
  if (console.log("[detectConflicts] Checking events count:", t.length, "Timezone:", s), console.log("[detectConflicts] Events data:", t.map((n) => ({ id: n.id, title: n.title, start: n.start, end: n.end }))), t.length < 2) return [];
  const a = [], o = t.map((n) => ({
    ...n,
    _mStart: f.tz(n.start, s),
    _mEnd: f.tz(n.end, s)
  ...
  return console.log("[detectConflicts] Conflicts found:", a.length), a;
};
class rt {
  plugins = [];
  context;
  constructor(s = [], a) {
    this.plugins = s, this.context = a;
  }
  setContext(s) {
    this.context = s;
  }
  init() {
    this.plugins.forEach((s) => {
      s.init && s.init(this.context);
    });
  }
  triggerBeforeRender() {
    this.plugins.forEach((s) => {
      s.hooks?.beforeRender && s.hooks.beforeRender(this.context);
    });
  }
  triggerOnEventRender(s, a) {
    this.plugins.forEach((o) => {
      o.hooks?.onEventRender && o.hooks.onEventRender(s, a);
    });
  }
  triggerOnEventClick(s) {
    this.plugins.forEach((a) => {
      a.hooks?.onEventClick && a.hooks.onEventClick(s);
    });
  }
  triggerAfterRender() {
    this.plugins.forEach((s) => {
      s.hooks?.afterRender && s.hooks.afterRender(this.context);
    });
  }
  triggerOnEventChange(s) {
    this.plugins.forEach((a) => {
      a.hooks?.onEventChange && a.hooks.onEventChange(s);
    });
  }
  triggerValidateSave(s) {
    const a = [];
    return this.plugins.forEach((o) => {
      if (o.hooks?.validateSave) {
        const n = o.hooks.validateSave(s, this.context);
        n && a.push(n);
      }
    }), a;
  }
}
const mt = ({ onChange: t, timeFormat: s, slotInterval: a, timezone: o, enabledTimeSlots: n, disabledTimeSlots: k, enabledTimeInterval: u, disableTimeInterval: i, checkIsSlotEnabled: m }) => {
  const h = b(() => {
    const g = [], A = f.tz(o).startOf("day");
    for (let D = 0; D < 1440; D += a)
      g.push(A.clone().add(D, "minutes"));
    return g;
  }, [a, o]);
...
}, ht = ({ value: t, onChange: s, dateFormat: a, timezone: o }) => {
  const [n, k] = B(() => f.tz(t, a, o).isValid() ? f.tz(t, a, o) : f.tz(o)), u = b(() => {
  ...
  return /* @__PURE__ */ l("div", { className: "absolute z-[70] bg-white border rounded shadow-xl mt-1 p-3 w-64 animate-fadeIn", children: [
    ...
    /* @__PURE__ */ e("div", { className: "grid grid-cols-7 gap-1", children: u.map((i, m) => /* @__PURE__ */ e(
      "div",
      {
        onClick: () => s(i.format(a)),
        className: `p-1.5 text-xs rounded cursor-pointer transition-colors ${i.isSame(n, "month") ? "hover:bg-blue-100" : "text-gray-300"} ${t === i.format(a) ? "bg-blue-600 text-white" : ""}`,
        children: i.date()
      },
      m
    )) })
  ...
  enabledTimeInterval: ie,
  disableTimeInterval: fe
}) => {
  const [P, I] = B([]), [L, T] = B(null);
  return t ? /* @__PURE__ */ l("div", { className: "calendrix-modal-overlay animate-fadeIn", children: [
    /* @__PURE__ */ l("div", { className: "calendrix-modal-content animate-scaleIn", children: [
      /* @__PURE__ */ l("div", { className: "flex justify-between items-center mb-6", children: [
        /* @__PURE__ */ e("h3", { className: "calendrix-modal-title", children: a ? "Edit Event" : "Create Event" }),
        /* @__PURE__ */ e("button", { onClick: s, className: "text-gray-400 hover:text-gray-600 transition-colors", children: /* @__PURE__ */ e("svg", { className: "w-6 h-6", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ e("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "2", d: "M6 18L18 6M6 6l12 12" }) }) })
      ] }),
      k?.map((r) => /* @__PURE__ */ l("div", { className: "calendrix-field-container", children: [
        /* @__PURE__ */ e("label", { className: "calendrix-label-hardened", children: r.label }),
        (() => {
          switch (r.type) {
            case "textarea":
              return /* @__PURE__ */ e("textarea", { required: r.required, value: o[r.name] || "", onChange: (c) => n({ ...o, [r.name]: c.target.value }), className: "calendrix-input-hardened", rows: 3 });
            case "dropdown":
            case "singleSelect":
              return /* @__PURE__ */ l("select", { required: r.required, value: o[r.name] || "", onChange: (c) => n({ ...o, [r.name]: c.target.value }), className: "calendrix-input-hardened", children: [
                /* @__PURE__ */ e("option", { value: "", children: "Select..." }),
                r.options?.map((c) => /* @__PURE__ */ e("option", { value: c.value, children: c.label }, c.value))
              ] });
            case "multiselect":
              return /* @__PURE__ */ e("select", { multiple: !0, required: r.required, value: o[r.name] || [], onChange: (c) => {
                const w = Array.from(c.target.selectedOptions, (H) => H.value);
                n({ ...o, [r.name]: w });
              }, className: "calendrix-input-hardened h-[128px]", children: r.options?.map((c) => /* @__PURE__ */ e("option", { value: c.value, children: c.label }, c.value)) });
            case "radio":
              return /* @__PURE__ */ e("div", { className: "flex gap-[16px] mt-[4px]", children: r.options?.map((c) => /* @__PURE__ */ l("label", { className: "flex items-center gap-[8px] cursor-pointer text-[#374151]", children: [
                /* @__PURE__ */ e("input", { type: "radio", name: r.name, value: c.value, checked: o[r.name] === c.value, onChange: (w) => n({ ...o, [r.name]: w.target.value }), className: "w-[16px] h-[16px] text-[#3b82f6] focus:ring-[#3b82f6]" }),
                c.label
              ] }, c.value)) });
            case "boolean":
              return /* @__PURE__ */ e("input", { type: "checkbox", checked: !!o[r.name], onChange: (c) => n({ ...o, [r.name]: c.target.checked }), className: "w-[20px] h-[20px] mt-[4px] cursor-pointer accent-[#3b82f6] rounded" });
            case "attachment":
              return /* @__PURE__ */ e("input", { type: "file", required: r.required, onChange: (c) => {
                c.target.files && c.target.files.length > 0 && n({ ...o, [r.name]: c.target.files[0].name });
              }, className: "calendrix-input-hardened" });
            case "colorPicker":
              return /* @__PURE__ */ e("input", { type: "color", required: r.required, value: o[r.name] || "#000000", onChange: (c) => n({ ...o, [r.name]: c.target.value }), className: "w-[64px] h-[40px] p-[4px] border border-[#e5e7eb] rounded-lg cursor-pointer bg-white" });
            case "year":
              return /* @__PURE__ */ e("input", { type: "number", required: r.required, placeholder: "YYYY", value: o[r.name] || "", onChange: (c) => n({ ...o, [r.name]: c.target.value }), className: "calendrix-input-hardened" });
            case "day":
              return /* @__PURE__ */ e("input", { type: "number", required: r.required, min: "1", max: "31", placeholder: "DD", value: o[r.name] || "", onChange: (c) => n({ ...o, [r.name]: c.target.value }), className: "calendrix-input-hardened" });
            default:
              const Y = !!((r.type === "datetime-local" || r.type === "datetime" || r.type === "date" || r.type === "time") && (i || m)), R = (() => {
                const c = o[r.name];
                if (!c || !Y) return c || "";
                const w = f.tz(c, u);
                return w.isValid() ? r.type === "datetime-local" || r.type === "datetime" ? w.format(`${i || "YYYY-MM-DD"} ${m || "HH:mm"}`) : r.type === "date" ? w.format(i || "YYYY-MM-DD") : r.type === "time" ? w.format(m || "HH:mm") : c : c || "";
              })();
              return /* @__PURE__ */ l("div", { className: "relative", children: [
                /* @__PURE__ */ e(
                  "input",
                  {
                    ...
                    value: R,
                    onChange: (c) => n({ ...o, [r.name]: c.target.value }),
                    onClick: () => {
                      Y && T({ name: r.name, type: r.type === "time" ? "time" : "date" });
                    },
                ...
                Y && /* @__PURE__ */ e(
                  "div",
                  {
                    ...
                    onClick: () => {
                      T({ name: r.name, type: r.type === "time" ? "time" : "date" });
                    },
                    children: /* @__PURE__ */ l("svg", { xmlns: "http://www.w3.org/2000/svg", width: "18", height: "18", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [
                ...
                Y && L?.name === r.name && L.type === "date" && /* @__PURE__ */ l(ne, { children: [
                  /* @__PURE__ */ e("div", { className: "fixed inset-0 z-[60]", onClick: () => T(null) }),
                  /* @__PURE__ */ e(
                    ht,
                    {
                      ...
                      onChange: (c) => {
                        ...
                        if (r.type === "datetime-local" || r.type === "datetime") {
                          const H = o[r.name], p = f.tz(H, `${i || "YYYY-MM-DD"} ${m || "HH:mm"}`, u), G = p.isValid() ? p.format(m || "HH:mm") : f().format(m || "HH:mm");
                          w = `${c} ${G}`, T({ name: r.name, type: "time" });
                        } else
                          T(null);
                        n({ ...o, [r.name]: w });
                      },
                ...
                Y && L?.name === r.name && L.type === "time" && /* @__PURE__ */ l(ne, { children: [
                  /* @__PURE__ */ e("div", { className: "fixed inset-0 z-[60]", onClick: () => T(null) }),
                  /* @__PURE__ */ e(
                    mt,
                    {
                      ...
                      onChange: (c) => {
                        let w = c;
                        if (r.type === "datetime-local" || r.type === "datetime") {
                          const H = o[r.name], p = f.tz(H, `${i || "YYYY-MM-DD"} ${m || "HH:mm"}`, u);
                          w = `${p.isValid() ? p.format(i || "YYYY-MM-DD") : f().format(i || "YYYY-MM-DD")} ${c}`;
                        }
                        n({ ...o, [r.name]: w }), T(null);
                      },
                      timeFormat: m || "HH:mm",
      ...
      /* @__PURE__ */ l("div", { className: "flex justify-between mt-4", children: [
        a && A && /* @__PURE__ */ e(
          "button",
          {
            onClick: () => {
        ...
        /* @__PURE__ */ l("div", { className: "flex gap-2 ml-auto", children: [
          /* @__PURE__ */ e(
            "button",
            {
              onClick: s,
          ...
          /* @__PURE__ */ e(
            "button",
            {
              onClick: () => {
                try {
                  const r = (c) => {
                    if (console.log("[EventFormModal] parseInTimezone input:", c), !c) return f().utc().toISOString();
                    if (c.includes("T") && (c.includes("Z") || c.split(":").length > 2)) {
                      const H = f(c).toISOString();
                      return console.log("[EventFormModal] recognized ISO:", H), H;
                    }
                    if (i && m) {
                      const H = f.tz(c, `${i} ${m}`, u);
                      if (H.isValid()) {
                        const p = H.toISOString();
                        return console.log("[EventFormModal] parsed via custom format:", p), p;
                      }
                    }
                    const w = f.tz(c, "YYYY-MM-DDTHH:mm", u);
                    if (w.isValid()) {
                      const H = w.toISOString();
                      return console.log("[EventFormModal] parsed via fallback format:", H), H;
                    }
                  ...
                  const R = D.triggerValidateSave(Y);
                  if (R.length > 0) {
                    I(R);
                    return;
                  }
    ...
    P.length > 0 && /* @__PURE__ */ e("div", { className: "fixed inset-0 bg-black/60 flex items-center justify-center z-[60] animate-fadeIn", children: /* @__PURE__ */ l(
      "div",
      {
        className: "bg-white rounded-2xl p-8 w-[450px] shadow-2xl border transform animate-scaleIn",
        style: {
          borderColor: N?.theme?.borderColor || "#fee2e2",
          backgroundColor: N?.theme?.backgroundColor || "#fff"
        },
        children: [
          N?.renderHeader ? N.renderHeader(N.title || "Conflict Detected", N.theme || { primaryColor: "#dc2626", secondaryColor: "#ef4444", backgroundColor: "#fff", textColor: "#1f2937", borderColor: "#fee2e2" }) : /* @__PURE__ */ l("div", { className: "flex items-center gap-4 mb-6", style: { color: N?.theme?.primaryColor || "#dc2626" }, children: [
            /* @__PURE__ */ e("div", { className: "w-12 h-12 rounded-full bg-red-50 flex items-center justify-center flex-shrink-0", style: { backgroundColor: N?.theme?.secondaryColor + "10" || "#fef2f2" }, children: N?.theme?.icon || /* @__PURE__ */ e("span", { className: "text-2xl", children: "⚠️" }) }),
            /* @__PURE__ */ l("div", { children: [
              /* @__PURE__ */ e("h4", { className: "text-xl font-bold", children: N?.title || "Conflict Detected" }),
              /* @__PURE__ */ e("p", { className: "text-sm opacity-70", children: N?.description || "Overlapping schedule found" })
            ] })
          ] }),
          /* @__PURE__ */ e("div", { className: "bg-gray-50 rounded-xl p-4 mb-6 max-h-60 overflow-y-auto border border-gray-100", style: { backgroundColor: N?.theme?.backgroundColor === "#fff" ? "#f9fafb" : N?.theme?.backgroundColor }, children: N?.renderDetails ? N.renderDetails(P) : P.map((r, Y) => /* @__PURE__ */ e("div", { className: "mb-3 last:mb-0", children: /* @__PURE__ */ e("div", { className: "text-sm font-medium text-gray-800 whitespace-pre-line leading-relaxed", style: { color: N?.theme?.textColor || "#1f2937" }, children: r }) }, Y)) }),
          N?.renderFooter ? N.renderFooter(() => I([]), N.theme || { primaryColor: "#dc2626", secondaryColor: "#ef4444", backgroundColor: "#fff", textColor: "#1f2937", borderColor: "#fee2e2" }) : /* @__PURE__ */ e("div", { className: "flex gap-3", children: /* @__PURE__ */ e(
            "button",
            {
              onClick: () => I([]),
              className: "flex-1 px-6 py-3 text-white font-semibold rounded-xl transition-all shadow-lg",
              style: {
                backgroundColor: N?.theme?.primaryColor || "#dc2626",
                boxShadow: `0 10px 15px -3px ${N?.theme?.primaryColor}40` || "0 10px 15px -3px rgba(220, 38, 38, 0.4)"
              },
              children: N?.buttonText || "I'll Fix It"
            }
          ) })
        ]
      }
  ...
  showEmptyState: N = !0,
  enabledTimeSlots: ee,
  disabledTimeSlots: de,
  enabledTimeInterval: ae,
  disableTimeInterval: ie,
  emptyStateContent: fe,
  emptyStateContentPopup: P,
  futureDaysOnly: I,
  pastDaysOnly: L,
  currentDayOnly: T,
  ...
  onDeleteEvent: c,
  formFields: w,
  onlyCreateEditRequired: H = !0,
  plugins: p,
  conflictTemplate: G,
  conflictThemeVariant: ve,
  calendarTheme: ue,
  calendarThemeVariant: le
}) => {
  const V = Fe(null), X = Fe(null), [De, J] = B(() => f.tz(u || /* @__PURE__ */ new Date(), t)), Ce = u !== void 0 ? u : De, W = (d) => {
    i ? i(d) : J(d);
  }, [K, te] = B(() => m || []), z = m !== void 0 ? m : K, se = Y || ((d) => te((v) => [...v, d])), x = R || ((d) => te((v) => v.map((M) => M.id === d.id ? d : M))), _ = c || ((d) => te((v) => v.filter((M) => M.id !== d))), re = w || [
  ...
  ], Q = b(() => ue || (le && ge[le] ? ge[le] : ge.classic_light), [ue, le]), Ye = b(() => ({
  ...
  const y = b(
    () => $e(Ce, t),
    [Ce, t]
  ), pe = b(() => f.utc().tz(t), [t]), we = y.isSame(pe, "day"), [S, q] = B(!1), [ce, me] = B(null), [Ne, j] = B({}), he = I && L, _e = !he && I && y.isBefore(f().tz(t), "day") || T && y.isBefore(f().tz(t), "day"), Ae = !he && L && y.isAfter(f().tz(t), "day") || T && y.isAfter(f().tz(t), "day"), Re = _e || Ae || T && !we, C = b(
    () => st(ae),
    [ae]
  ), E = () => {
    q(!1), me(null), j({});
    ...
    O.triggerBeforeRender();
    const d = dt(F, y, a);
    return O.triggerAfterRender(), d;
  }, [F, y, a, O]), xe = !T && (he || !(I && y.isSameOrBefore(pe, "day"))), Se = !T && (he || !(L && y.isSameOrAfter(pe, "day"))), He = () => {
    xe && W(y.clone().subtract(1, "day"));
  }, Be = () => {
    Se && W(y.clone().add(1, "day"));
  }, qe = () => W(f.utc().tz(t)), [Ee, Me] = B(null), [Oe, nt] = B(null), Ue = (d) => Math.round(d / a) * a;
  Le(() => {
    const d = (M) => {
      if (!X.current) return;
      const ye = X.current.getBoundingClientRect(), je = (M.clientY - ye.top) / Z * a;
      if (Ee) {
        const ke = Ue(je), be = y.clone().startOf("day").add(ke, "minutes"), Te = Ee.end.diff(
          Ee.start,
          "minutes"
        ), Pe = {
          ...Ee,
          start: be,
          end: be.clone().add(Te, "minutes")
        };
        h?.(Pe), O.triggerOnEventChange(Pe);
      }
      if (Oe) {
        const ke = Ue(je), be = y.clone().startOf("day").add(ke, "minutes");
        if (be.isAfter(Oe.start)) {
          const Te = {
            ...Oe,
            end: be
          };
          h?.(Te), O.triggerOnEventChange(Te);
        }
      }
    }, v = () => {
      Me(null), nt(null);
    };
    return window.addEventListener("mousemove", d), window.addEventListener("mouseup", v), () => {
  ...
  const Ze = (d, v) => {
    ...
    if (v)
      ...
      const We = X.current.getBoundingClientRect(), ke = (d.clientY - We.top) / Z * a, be = Ue(ke);
      M = y.clone().startOf("day").add(be, "minutes");
    } else
      return;
    if (!Ve(M)) return;
    const ye = M.clone().add(a, "minutes");
    me(null), j({
      start: M.format(`${o || "YYYY-MM-DD"} ${n || "HH:mm"}`),
      ...
      V.current && requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          if ($.length > 0)
            ...
            const M = Xe.findIndex((ye) => Ve(ye));
            M !== -1 && (v = M * Z);
          }
          if (v > 0) {
            const M = Math.max(0, v - Z), ye = V.current.scrollTop, We = M - ye, je = 2e3;
            let ke = null;
            const be = (Te) => {
              ke === null && (ke = Te);
              const Pe = Te - ke, Ie = Pe / je, ft = Ie < 0.5 ? 4 * Ie * Ie * Ie : 1 - Math.pow(-2 * Ie + 2, 3) / 2;
              V.current && (V.current.scrollTop = ye + We * ft, Pe < je ? requestAnimationFrame(be) : V.current.scrollTop = M);
            };
  ...
  const Ge = /* @__PURE__ */ l("div", { className: "text-center flex flex-col items-center", children: [
    /* @__PURE__ */ e("h2", { className: "text-xl font-semibold px-4 py-1 rounded-full text-white", style: we ? { backgroundColor: "var(--calendar-primary)" } : { color: "var(--calendar-text)" }, children: y.format(o) }),
    s && /* @__PURE__ */ l("p", { className: "text-xs text-gray-500 mt-1", children: [
      "GMT",
      y.format("Z"),
      " • ",
      t
    ...
    "button",
    {
      onClick: He,
      disabled: !xe,
      className: `px-3 py-1 rounded ${xe ? "hover:bg-gray-200" : "opacity-50 cursor-not-allowed"}`,
      children: "◀"
    }
  ), Ke = /* @__PURE__ */ e(
    "button",
  ...
  ), Qe = /* @__PURE__ */ l("div", { className: "flex items-center gap-2", children: [
    Je,
    Ke
  ] }), it = D ? D({
    goToPreviousDay: He,
    ...
    currentDate: y,
    timezone: t,
    timezoneLabelInclude: s
  }) : null;
  return /* @__PURE__ */ l("div", { className: "flex flex-col flex-1 h-full w-full min-h-0 no-scrollbar", style: { ...Ye, backgroundColor: "var(--calendar-bg)", color: "var(--calendar-text)" }, children: [
    D != null ? /* @__PURE__ */ e("div", { children: it }, "custom-nav-wrapper") : /* @__PURE__ */ l("div", { className: "sticky top-0 z-20 border-b px-6 py-4 flex items-center min-h-[80px]", style: { backgroundColor: "var(--calendar-bg)", borderColor: "var(--calendar-grid)" }, children: [
      g === "left" && /* @__PURE__ */ l(ne, { children: [
      ...
      g === "center" && /* @__PURE__ */ l(ne, { children: [
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-start items-center" }),
        /* @__PURE__ */ l("div", { className: "flex-1 flex justify-center items-center gap-4", children: [
          Je,
          Ge,
          Ke
        ] }),
      ...
      g === "right" && /* @__PURE__ */ l(ne, { children: [
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-start items-center" }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-center items-center", children: Ge }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-end items-center gap-4", children: Qe })
      ] })
    ] }),
    k === !1 ? /* @__PURE__ */ l("div", { className: "flex flex-col flex-1 min-h-0 m-5 p-4 overflow-y-auto no-scrollbar relative", children: [
      H && /* @__PURE__ */ e("div", { className: "flex justify-end mb-4", children: /* @__PURE__ */ e(
        "button",
        {
          onClick: () => {
            me(null);
            const d = y.clone().hour(9).minute(0), v = d.clone().add(a, "minutes");
            j({
              start: d.format("YYYY-MM-DDTHH:mm"),
              end: v.format("YYYY-MM-DDTHH:mm")
            }), q(!0);
          },
      ...
      F.length === 0 ? /* @__PURE__ */ e("div", { className: "flex flex-col items-center justify-center flex-1 h-full min-h-[200px] border-2 border-dashed rounded-xl", style: { borderColor: "var(--calendar-grid)" }, children: /* @__PURE__ */ e("p", { className: "text-gray-500 font-medium mb-4", children: fe || "No events scheduled" }) }) : /* @__PURE__ */ e("div", { className: "flex flex-col gap-3 pb-8", children: F.sort((d, v) => f(d.start).diff(f(v.start))).map((d) => /* @__PURE__ */ l(
        "div",
        {
          onClick: (v) => {
            v.stopPropagation(), O.triggerOnEventClick(d), H && (me(d), j({
              ...d,
              start: f(d.start).tz(t).format("YYYY-MM-DDTHH:mm"),
              end: f(d.end).tz(t).format("YYYY-MM-DDTHH:mm")
            }), q(!0));
          },
          ...
          children: [
            /* @__PURE__ */ l("div", { className: "flex justify-between items-start", children: [
              /* @__PURE__ */ e("div", { className: "font-semibold text-base", children: d.title }),
              /* @__PURE__ */ e(
                "button",
                {
                  onClick: (v) => {
                    v.stopPropagation(), _(d.id);
                  },
            ...
            /* @__PURE__ */ l("div", { className: "text-sm opacity-80 mt-1", children: [
              f(d.start).tz(t).format(n),
              " - ",
              f(d.end).tz(t).format(n)
            ] })
          ]
        },
        d.id
      )) })
    ] }) : /* @__PURE__ */ e("div", { ref: V, className: "flex-1 overflow-y-auto no-scrollbar", children: /* @__PURE__ */ l("div", { className: "flex min-h-full", children: [
      /* @__PURE__ */ e("div", { className: "w-24 flex-shrink-0", style: { backgroundColor: "var(--calendar-secondary-bg)" }, children: Xe.map((d, v) => {
        const M = Ve(d);
        return /* @__PURE__ */ e(
          "div",
          {
            ...
            children: /* @__PURE__ */ e("span", { className: "absolute top-1/2 -translate-y-1/2 right-2", children: d.format(n) })
          },
          v
        );
      }) }),
      /* @__PURE__ */ l("div", { ref: X, className: "flex-1 relative", onDoubleClick: (d) => Ze(d), children: [
        Xe.map((d, v) => {
          const M = Ve(d);
          return /* @__PURE__ */ e(
            "div",
        ...
        $.map((d) => /* @__PURE__ */ l(
          ...
          {
            ref: (v) => {
              v && O.triggerOnEventRender(d, v);
            },
            onMouseDown: () => Me(d),
            onDoubleClick: (v) => {
              v.stopPropagation(), O.triggerOnEventClick(d), x && (me(d), j({
                ...d,
                start: f(d.start).tz(t).format(`${o || "YYYY-MM-DD"} ${n || "HH:mm"}`),
                end: f(d.end).tz(t).format(`${o || "YYYY-MM-DD"} ${n || "HH:mm"}`)
              }), q(!0));
            },
            ...
            style: {
              top: d.top,
              height: d.height,
              left: `${d.columnIndex / d.columnCount * 100}%`,
              width: `${100 / d.columnCount}%`,
              backgroundColor: "var(--calendar-event-bg)",
              color: "var(--calendar-event-text)"
            },
            children: [
        ...
        N && $.length === 0 && !Re && /* @__PURE__ */ e(
          "div",
          {
            ...
            style: {
              top: C.startMinutes / a * Z,
              height: (C.endMinutes - C.startMinutes) / a * Z,
              cursor: Re ? "not-allowed" : "pointer"
            },
            children: /* @__PURE__ */ l("div", { className: "bg-white shadow-xl rounded-2xl px-8 py-6 border text-center animate-scaleIn", children: [
              /* @__PURE__ */ e("p", { className: "text-gray-600 font-medium", children: fe || "No events scheduled" }),
              P || /* @__PURE__ */ e(
                "button",
                {
                  onClick: () => {
        ...
        we && /* @__PURE__ */ e(
          "div",
          {
            ...
            style: {
              ...
              top: (pe.hours() * 60 + pe.minutes()) / a * Z
            }
          }
        )
      ] })
    ...
    H && /* @__PURE__ */ e(
      ot,
      {
        ...
        onDeleteEvent: _,
        pluginManager: O,
        conflictTemplate: G || (ve ? xt[ve] : void 0),
        slotInterval: a,
        enabledTimeSlots: ee,
        disabledTimeSlots: de,
        enabledTimeInterval: ae,
  ...
  const R = Fe(null), [c, w] = B(() => f.tz(k || /* @__PURE__ */ new Date(), t)), H = k !== void 0 ? k : c, p = (C) => {
    u ? u(C) : w(C);
  }, [G, ve] = B(() => i || []), ue = i !== void 0 ? i : G, le = ae || ((C) => ve((E) => [...E, C])), V = ie || ((C) => ve((E) => E.map((U) => U.id === C.id ? C : U))), X = fe || ((C) => ve((E) => E.filter((U) => U.id !== C))), De = P || [
    { name: "title", label: "Event Title", type: "text", required: !0 },
    { name: "description", label: "Description", type: "textarea" },
    { name: "start", label: "Start Time", type: "datetime-local", required: !0 },
    { name: "end", label: "End Time", type: "datetime-local", required: !0 }
  ], J = b(() => r || (Y && ge[Y] ? ge[Y] : ge.classic_light), [r, Y]), Ce = b(() => ({
    "--calendar-primary": J.primaryColor,
    "--calendar-bg": J.backgroundColor,
    "--calendar-secondary-bg": J.secondaryBackgroundColor,
    "--calendar-grid": J.gridColor,
    "--calendar-text": J.textColor,
    "--calendar-secondary-text": J.secondaryTextColor,
    "--calendar-accent": J.accentColor,
    "--calendar-event-bg": J.eventDefaultColor,
    "--calendar-event-text": J.eventDefaultTextColor
  }), [J]), W = b(() => ue ?? [], [ue]), K = b(() => new rt(T, {
    timezone: t,
    slotInterval: a,
    events: W,
    onEventChange: m,
  ...
  const te = b(
    () => $e(H, t),
    [H, t]
  ), z = b(() => te.clone().startOf("week"), [te]), se = b(() => {
    const C = [];
    for (let E = 0; E < 7; E++)
      C.push(z.clone().add(E, "days"));
    return C;
  }, [z]), x = b(() => f.utc().tz(t), [t]), [_, re] = B(!1), [Q, Ye] = B(null), [oe, O] = B({}), y = b(
    () => st(ee),
    [ee]
  ), pe = () => {
    re(!1), Ye(null), O({});
  }, we = () => p(te.clone().subtract(1, "week")), S = () => p(te.clone().add(1, "week")), q = () => p(f.utc().tz(t)), ce = b(
    () => ct(z, a),
    [z, a]
  ), me = (C) => et(
    C,
    ...
    const C = se.map((E) => {
      const U = tt(W, E, t).filter((F) => !F.allDay);
      return dt(U, E, a);
    });
    return K.triggerAfterRender(), C;
  }, [se, W, t, a, K]);
  at(() => {
      ...
      R.current && requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          if (E !== 1 / 0)
            ...
            const F = ce.findIndex(($) => me($));
            F !== -1 && (U = F * Z);
          }
          if (U > 0) {
            const F = Math.max(0, U - Z), $ = R.current.scrollTop, xe = F - $, Se = 2e3;
            let He = null;
            const Be = (qe) => {
              He === null && (He = qe);
              const Ee = qe - He, Me = Ee / Se, Oe = Me < 0.5 ? 4 * Me * Me * Me : 1 - Math.pow(-2 * Me + 2, 3) / 2;
              R.current && (R.current.scrollTop = $ + xe * Oe, Ee < Se ? requestAnimationFrame(Be) : R.current.scrollTop = F);
            };
            requestAnimationFrame(Be);
          }
        });
  ...
  const j = /* @__PURE__ */ l("div", { className: "text-center flex flex-col items-center", children: [
    /* @__PURE__ */ e("h2", { className: "text-xl font-semibold", children: z.format("MMMM YYYY") }),
    s && /* @__PURE__ */ l("p", { className: "text-xs text-gray-500 mt-1", children: [
      "GMT",
      te.format("Z"),
      " • ",
  ...
  ), Ae = /* @__PURE__ */ l("div", { className: "flex items-center gap-2", children: [
    he,
    _e
  ] }), Re = g ? g({
    goToPreviousDay: we,
  ...
  return /* @__PURE__ */ l("div", { className: "flex flex-col flex-1 h-full w-full min-h-0 no-scrollbar", style: { ...Ce, backgroundColor: "var(--calendar-bg)", color: "var(--calendar-text)" }, children: [
    g != null ? /* @__PURE__ */ e("div", { children: Re }, "custom-nav-wrapper") : /* @__PURE__ */ l("div", { className: "sticky top-0 z-20 border-b px-6 py-4 flex items-center min-h-[80px]", style: { backgroundColor: "var(--calendar-bg)", borderColor: "var(--calendar-grid)" }, children: [
      h === "left" && /* @__PURE__ */ l(ne, { children: [
      ...
      h === "center" && /* @__PURE__ */ l(ne, { children: [
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-start items-center" }),
        /* @__PURE__ */ l("div", { className: "flex-1 flex justify-center items-center gap-4", children: [
          he,
          j,
      ...
      h === "right" && /* @__PURE__ */ l(ne, { children: [
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-start items-center" }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-center items-center", children: j }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-end items-center gap-4", children: Ae })
      ] })
    ] }),
    /* @__PURE__ */ l("div", { className: "flex flex-1 flex-col min-h-0 m-5 mt-2 overflow-hidden", style: { backgroundColor: "var(--calendar-bg)" }, children: [
      /* @__PURE__ */ l("div", { className: "flex border-b", style: { borderColor: "var(--calendar-grid)" }, children: [
        /* @__PURE__ */ e("div", { className: "w-24 flex-shrink-0" }),
        /* @__PURE__ */ e("div", { className: "flex flex-1 min-w-[700px]", children: se.map((C, E) => /* @__PURE__ */ l("div", { className: "flex-1 text-center font-medium py-3", style: { color: "var(--calendar-text)" }, children: [
          /* @__PURE__ */ e("div", { className: "text-xs uppercase tracking-wider", style: { color: "var(--calendar-secondary-text)" }, children: C.format("ddd") }),
          /* @__PURE__ */ e(
            "div",
            {
              className: `text-2xl mt-1 w-10 h-10 flex items-center justify-center mx-auto rounded-full ${C.isSame(x, "day") ? "text-white" : ""}`,
              style: C.isSame(x, "day") ? { backgroundColor: "var(--calendar-primary)" } : {},
              children: C.format("D")
            }
          )
        ] }, E)) })
      ] }),
      /* @__PURE__ */ e("div", { ref: R, className: "flex-1 overflow-y-auto overflow-x-hidden no-scrollbar relative", children: /* @__PURE__ */ l("div", { className: "flex min-h-full", children: [
        /* @__PURE__ */ e("div", { className: "w-24 flex-shrink-0 z-10 sticky left-0 shadow-[1px_0_5px_rgba(0,0,0,0.02)]", style: { backgroundColor: "var(--calendar-bg)" }, children: ce.map((C, E) => {
          const U = me(C);
          return /* @__PURE__ */ e(
            "div",
            {
              ...
              children: /* @__PURE__ */ e("span", { className: "absolute top-1/2 -translate-y-1/2 right-2 px-1", style: { backgroundColor: "var(--calendar-bg)" }, children: C.format(n) })
            },
            E
          );
        }) }),
        /* @__PURE__ */ l("div", { className: "flex flex-1 relative min-w-[700px]", style: { backgroundColor: "var(--calendar-bg)" }, children: [
          /* @__PURE__ */ e("div", { className: "absolute inset-0 pointer-events-none", children: ce.map((C, E) => /* @__PURE__ */ e(
            "div",
            {
          ...
          se.map((C, E) => {
            const U = Ne[E], F = C.isSame(x, "day");
            return /* @__PURE__ */ l("div", { className: "flex-1 relative", children: [
              /* @__PURE__ */ e("div", { className: "absolute inset-0 z-0 bg-transparent cursor-pointer", onDoubleClick: () => {
                I && (O({ start: C.clone().hour(9).format(`${o || "YYYY-MM-DD"} ${n || "HH:mm"}`) }), re(!0));
              } }),
              F && /* @__PURE__ */ e(
                "div",
                {
              ...
              A && U.length === 0 && /* @__PURE__ */ e(
                "div",
                {
                  ...
                  style: {
                    top: y.startMinutes / a * Z,
                    height: (y.endMinutes - y.startMinutes) / a * Z
                  },
                  children: /* @__PURE__ */ e("div", { className: "text-xs font-medium", style: { color: "var(--calendar-secondary-text)" }, children: "Clear" })
                }
              ),
              /* @__PURE__ */ e("div", { className: "absolute inset-0 z-10 pointer-events-none", children: U.map(($) => /* @__PURE__ */ l(
                "div",
                {
                  onDoubleClick: (xe) => {
                    xe.stopPropagation(), K.triggerOnEventClick($), I && (Ye($), O({
                      ...$,
                      start: f($.start).tz(t).format(`${o || "YYYY-MM-DD"} ${n || "HH:mm"}`),
                      end: f($.end).tz(t).format(`${o || "YYYY-MM-DD"} ${n || "HH:mm"}`)
                    }), re(!0));
                  },
                  ref: (xe) => {
                    xe && K.triggerOnEventRender($, xe);
                  },
                  ...
                  children: [
                    /* @__PURE__ */ e("div", { className: "truncate", children: $.title }),
                    X && I && /* @__PURE__ */ e(
                      "button",
                      {
                        onClick: (xe) => {
                          xe.stopPropagation(), X($.id);
                        },
  ...
  plugins: de,
  calendarTheme: ae,
  calendarThemeVariant: ie
}) => {
  const [fe, P] = B(() => f.tz(n || /* @__PURE__ */ new Date(), t)), I = n !== void 0 ? n : fe, L = (S) => {
    k ? k(S) : P(S);
  }, [T, r] = B(() => u || []), Y = u !== void 0 ? u : T, R = g || ((S) => r((q) => [...q, S])), c = A || ((S) => r((q) => q.map((ce) => ce.id === S.id ? S : ce))), w = D || ((S) => r((q) => q.filter((ce) => ce.id !== S))), H = N || [
    { name: "title", label: "Event Title", type: "text", required: !0 },
    { name: "description", label: "Description", type: "textarea" },
    { name: "start", label: "Start Time", type: "datetime-local", required: !0 },
    { name: "end", label: "End Time", type: "datetime-local", required: !0 }
    ...
    () => $e(I, t),
    [I, t]
  ), G = b(() => ae || (ie && ge[ie] ? ge[ie] : ge.classic_light), [ae, ie]), ve = b(() => ({
    ...
    onEditEvent: c,
    onDeleteEvent: w
  }), [de, t, ue, i, R, c, w]), [V, X] = B(!1), [De, J] = B(null), [Ce, W] = B({}), K = b(() => p.clone().startOf("month"), [p]), te = b(() => p.clone().endOf("month"), [p]), z = b(() => K.clone().startOf("week"), [K]), se = b(() => te.clone().endOf("week"), [te]), x = b(() => {
    const S = [];
    let q = z.clone();
    for (; q.isBefore(se) || q.isSame(se, "day"); )
      S.push(q.clone()), q.add(1, "day");
    return S;
  }, [z, se]), _ = () => L(p.clone().subtract(1, "month")), re = () => L(p.clone().add(1, "month")), Q = () => L(f.utc().tz(t)), Ye = () => {
    X(!1), J(null), W({});
  }, oe = /* @__PURE__ */ l("div", { className: "text-center flex flex-col items-center", children: [
    /* @__PURE__ */ e("h2", { className: "text-xl font-semibold", children: p.format(a) }),
    s && /* @__PURE__ */ l("p", { className: "text-xs mt-1", style: { color: "var(--calendar-secondary-text)" }, children: [
      "GMT",
      p.format("Z"),
      " • ",
      t
    ...
    /* @__PURE__ */ e("button", { onClick: Q, className: "mt-1 text-sm font-medium", style: { color: "var(--calendar-primary)" }, children: "Today" })
  ] }), O = /* @__PURE__ */ e("button", { onClick: _, className: "px-3 py-1 rounded hover:opacity-70 transition-opacity", style: { color: "var(--calendar-text)" }, children: "◀" }), y = /* @__PURE__ */ e("button", { onClick: re, className: "px-3 py-1 rounded hover:opacity-70 transition-opacity", style: { color: "var(--calendar-text)" }, children: "▶" }), pe = /* @__PURE__ */ l("div", { className: "flex items-center gap-2", children: [
    O,
    y
  ] }), we = h ? h({
    goToPreviousDay: _,
  ...
  return /* @__PURE__ */ l("div", { className: "flex flex-col flex-1 h-full w-full min-h-0 no-scrollbar", style: { ...ve, backgroundColor: "var(--calendar-bg)", color: "var(--calendar-text)" }, children: [
    h != null ? /* @__PURE__ */ e("div", { children: we }, "custom-nav-wrapper") : /* @__PURE__ */ l("div", { className: "sticky top-0 z-20 border-b px-6 py-4 flex items-center min-h-[80px]", style: { backgroundColor: "var(--calendar-bg)", borderColor: "var(--calendar-grid)" }, children: [
      m === "left" && /* @__PURE__ */ l(ne, { children: [
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-start items-center", children: pe }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-center items-center", children: oe }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-end items-center" })
      ] }),
      m === "center" && /* @__PURE__ */ l(ne, { children: [
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-start items-center" }),
        /* @__PURE__ */ l("div", { className: "flex-1 flex justify-center items-center gap-4", children: [
          O,
          oe,
      ...
      m === "right" && /* @__PURE__ */ l(ne, { children: [
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-start items-center" }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-center items-center", children: oe }),
    ...
    /* @__PURE__ */ e("div", { className: "flex-1 overflow-y-auto no-scrollbar", children: /* @__PURE__ */ e("div", { className: "grid grid-cols-7 min-h-full", children: x.map((S, q) => {
      const ce = S.isSame(p, "month"), me = S.isSame(f().tz(t), "day"), Ne = tt(ue, S, t);
      return /* @__PURE__ */ l(
        "div",
        {
          ...
          onDoubleClick: () => {
            if (ee) {
              const j = S.clone().hour(9).minute(0), he = j.clone().add(1, "hour");
              W({
                start: j.format(`${a || "YYYY-MM-DD"}T${o || "HH:mm"}`),
                end: he.format(`${a || "YYYY-MM-DD"}T${o || "HH:mm"}`)
              }), X(!0);
            }
          },
          children: [
            /* @__PURE__ */ e("div", { className: "flex justify-end pr-1", children: /* @__PURE__ */ e(
              "span",
              {
                ...
                style: me ? { backgroundColor: "var(--calendar-primary)" } : { color: ce ? "var(--calendar-text)" : "var(--calendar-secondary-text)" },
                children: S.date()
              }
            ) }),
            /* @__PURE__ */ l("div", { className: "flex flex-col gap-1 flex-1 overflow-hidden", children: [
              Ne.slice(0, 4).map((j) => /* @__PURE__ */ e(
                "div",
                {
                  onClick: (he) => {
                    he.stopPropagation(), le.triggerOnEventClick(j), ee && (J(j), W({
                      ...j,
                      start: f(j.start).tz(t).format(`${a || "YYYY-MM-DD"}T${o || "HH:mm"}`),
                      end: f(j.end).tz(t).format(`${a || "YYYY-MM-DD"}T${o || "HH:mm"}`)
                    }), X(!0));
                  },
                  ...
                  style: {
                    backgroundColor: "var(--calendar-event-bg)",
                    color: "var(--calendar-event-text)",
                    border: j.hasConflict ? "1px solid red" : "none"
                  },
                  title: j.title,
                  children: j.title
                },
              ...
              Ne.length > 4 && /* @__PURE__ */ l("div", { className: "text-[9px] px-1 font-bold", style: { color: "var(--calendar-secondary-text)" }, children: [
                "+",
                Ne.length - 4,
                " more"
              ] })
  ...
  const {
    ...
    timeFormat: N = "HH:mm",
    showTimeSlots: ee = !1,
    timezoneLabelInclude: de = !1
  } = t, [ae, ie] = B(() => f.tz(a || /* @__PURE__ */ new Date(), s)), fe = a !== void 0 ? a : ae, P = b(
    () => $e(fe, s),
    [fe, s]
  ), [I, L] = B(() => P.clone().startOf("month")), T = (x) => {
    o ? o(x) : ie(x), L(x.clone().startOf("month"));
  }, [r, Y] = B(() => n || []), R = n !== void 0 ? n : r, c = k || ((x) => Y((_) => [..._, x])), w = u || ((x) => Y((_) => _.map((re) => re.id === x.id ? x : re))), H = i || ((x) => Y((_) => _.filter((re) => re.id !== x))), p = b(() => m || (h && ge[h] ? ge[h] : ge.classic_light), [m, h]), G = b(() => ({
    "--calendar-primary": p.primaryColor,
    "--calendar-bg": p.backgroundColor,
    "--calendar-secondary-bg": p.secondaryBackgroundColor,
    "--calendar-grid": p.gridColor,
    "--calendar-text": p.textColor,
    "--calendar-secondary-text": p.secondaryTextColor,
    "--calendar-accent": p.accentColor,
    "--calendar-event-bg": p.eventDefaultColor,
    "--calendar-event-text": p.eventDefaultTextColor
  }), [p]), ve = I.clone().startOf("month"), ue = I.clone().endOf("month"), le = ve.clone().startOf("week"), V = ue.clone().endOf("week"), X = b(() => {
    const x = [];
    let _ = le.clone();
    for (; _.isBefore(V) || _.isSame(V, "day"); )
      x.push(_.clone()), _.add(1, "day");
    return x;
  }, [le, V]), De = () => L((x) => x.clone().subtract(1, "month")), J = () => L((x) => x.clone().add(1, "month")), Ce = () => T(f.utc().tz(s)), W = /* @__PURE__ */ l("div", { className: "text-center flex flex-col items-center", children: [
    /* @__PURE__ */ e("h2", { className: "text-xl font-semibold", style: { color: "var(--calendar-text)" }, children: P.format(D) }),
    de && /* @__PURE__ */ l("p", { className: "text-xs mt-1", style: { color: "var(--calendar-secondary-text)" }, children: [
      "GMT",
      P.format("Z"),
      " • ",
      s
  ...
  ] }), K = /* @__PURE__ */ e("button", { onClick: () => T(P.clone().subtract(1, "month")), className: "px-3 py-1 rounded hover:bg-gray-100 transition-colors", style: { color: "var(--calendar-text)" }, children: "◀" }), te = /* @__PURE__ */ e("button", { onClick: () => T(P.clone().add(1, "month")), className: "px-3 py-1 rounded hover:bg-gray-100 transition-colors", style: { color: "var(--calendar-text)" }, children: "▶" }), z = /* @__PURE__ */ l("div", { className: "flex items-center gap-2", children: [
    K,
    te
  ] }), se = A ? A({
    goToPreviousDay: () => T(P.clone().subtract(1, "month")),
    goToNextDay: () => T(P.clone().add(1, "month")),
    goToToday: Ce,
    dateNode: W,
    prevNode: K,
    nextNode: te,
    ...
    currentDate: P,
    timezone: s,
    timezoneLabelInclude: de
  }) : null;
  return /* @__PURE__ */ l("div", { className: "flex flex-col h-full w-full no-scrollbar", style: { ...G, backgroundColor: "var(--calendar-bg)" }, children: [
    A ? se : /* @__PURE__ */ l("div", { className: "sticky top-0 z-20 border-b px-6 py-4 flex items-center min-h-[80px]", style: { backgroundColor: "var(--calendar-bg)", borderColor: "var(--calendar-grid)" }, children: [
      g === "left" && /* @__PURE__ */ l(ne, { children: [
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-start items-center", children: z }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-center items-center", children: W }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-end items-center" })
      ] }),
      g === "center" && /* @__PURE__ */ l(ne, { children: [
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-start items-center" }),
      ...
      g === "right" && /* @__PURE__ */ l(ne, { children: [
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-start items-center" }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-center items-center", children: W }),
        /* @__PURE__ */ e("div", { className: "flex-1 flex justify-end items-center gap-4", children: z })
      ] })
    ] }),
    /* @__PURE__ */ l("div", { className: "flex flex-col md:flex-row flex-1 overflow-hidden border shadow-lg rounded-xl bg-white no-scrollbar m-4 mt-2", style: { borderColor: "var(--calendar-grid)" }, children: [
      /* @__PURE__ */ l(
        "div",
        {
          ...
          children: [
            /* @__PURE__ */ l("div", { className: "flex items-center justify-between mb-4", children: [
              ...
              /* @__PURE__ */ e("div", { className: "font-semibold text-sm", style: { color: "var(--calendar-text)" }, children: I.format("MMMM YYYY") }),
              /* @__PURE__ */ e(
                "button",
                {
                  onClick: J,
            ...
            /* @__PURE__ */ e("div", { className: "grid grid-cols-7 gap-1", children: X.map((x, _) => {
              const re = x.isSame(I, "month"), Q = x.isSame(P, "day"), Ye = x.isSame(f().tz(s), "day");
              let oe = "hover:bg-gray-200", O = { color: "var(--calendar-text)" }, y = {};
              return re || (O = { color: "var(--calendar-secondary-text)", opacity: 0.5 }), Q ? (oe = "", y = { backgroundColor: "var(--calendar-primary)", color: "white" }, O = { color: "white" }) : Ye && (y = { border: "2px solid var(--calendar-primary)", fontWeight: "bold" }, O = { color: "var(--calendar-primary)" }), /* @__PURE__ */ e(
                "button",
                {
                  onClick: () => T(x),
                  className: `h-8 w-8 flex items-center justify-center rounded-full text-xs transition-colors ${oe}`,
                  style: { ...O, ...y },
                  children: x.date()
                },
                _
      ...
      /* @__PURE__ */ e("div", { className: "flex-1 min-w-0 h-full overflow-hidden bg-white", children: /* @__PURE__ */ e(
        gt,
        {
          ...
          onEditEvent: w,
          onDeleteEvent: H,
          renderNavigation: () => /* @__PURE__ */ e(ne, {}),
          showTodayBelow: !1,
          dateFormat: D,
          timeFormat: N
        }
